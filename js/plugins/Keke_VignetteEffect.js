...
/*:
 * @target MZ
 * @plugindesc ビネット画像を生成・表示・操作する
 * @author ケケー
 * 
 * @help
 * 【ver.2.3.1】
 * 中抜き化した画像を表示するビネットエフェクト
 ...
 *
 * @arg レイヤー
 * @desc 配置するレイヤー。A はキャラより下、B はキャラより上。数字が大きいほど上に表示
 * @type select
 * @option レイヤーA1
 * @option レイヤーA2
 * @option レイヤーA3
 ...
 *
 * @param ダイヤ-線丸み
 * @desc ダイヤ形の直線を丸くする。5 なら 5ピクセル分 丸まる
 * @default 0
 *
 * @param ギザギザ-トゲ数X
 ...
 *
 * @param 拡縮アニメ-時間
 * @desc 拡縮アニメの一周時間。5 なら 5フレーム で一周
 * @default 120
 *
 * @param 拡縮アニメ-大きさ
 ...
 *
 * @param 明滅アニメ-時間
 * @desc 明滅アニメの一周時間。5 なら 5フレーム で一周
 * @default 120
 *
 * @param 明滅アニメ-大きさ
 ...
 *
 * @param 回転アニメ-時間
 * @desc 回転アニメの一周時間。5 なら 5フレーム で一周
 * @default 480
 *
 * @param 回転アニメ-大きさ
...
/*~struct~chaseParam:
* @param 対象キャラ-名前
* @desc 名前で追尾対象を選択。***→名前メモ検索、セルフ→イベント自身、プレイヤー→プレイヤー、全て→全キャラ。, で複数指定可
*
* @param 対象キャラ-ID
* @desc 追尾対象ID。1〜→イベントID。0→イベント自身、-1→レイヤー、-2→全キャラ。 , で複数指定可、~ でまとめて指定可
*
* @param 追尾解除
* @desc 追尾をやめる
* @type boolean
...
(() => {
    ...
    PluginManager.registerCommand(pluginName, "ビネット表示", args => {
        ...
        // ウェイトをセット
        const time = Number(args["フェードイン時間"]) || 1;
        if (eval(args["…動作中ウェイト"]) && time) { preter.wait(time); }
        // ビネットの開始
        startVignette(args);
    });
    ...
    PluginManager.registerCommand(pluginName, "アイリスエフェクト", args => {
        ...
        const preter = plcmPreter;
        // アイリスウェイトのセット
        const time = Number(args["動作時間"]) || 1;
        if (eval(args["…動作中ウェイト"]) && time) { setIrisWait(preter); }
        args.preter = preter;
        // アイリスエフェクトの開始
    ...
    PluginManager.registerCommand(pluginName, "ビネット操作", args => {
        ...
        // 操作ウェイトのセット
        const time = Number(args["動作時間"]) || 1;
        if (eval(args["…動作中ウェイト"]) && time) { setControlWait(preter); }
        args.preter = preter;
        // ビネット操作の開始
    ...
    PluginManager.registerCommand(pluginName, "ビネット削除", args => {
        ...
        const preter = plcmPreter;
        // ウェイトをセット
        const time = Number(args["フェードアウト時間"]) || 1;
        if (eval(args["…動作中ウェイト"]) && time) { preter.wait(time); }
        // ビネットの削除
        eraseVignette(args);
    });
    ...
    function setIrisWait(preter) {
        preter._irisEffectWaitKe = true;
    };
    
    
    // アイリスウェイトの解除
    function remIrisWait(preter) {
        preter._irisEffectWaitKe = false;
    };
    
    
    // 操作ウェイトのセット
    function setControlWait(preter) {
        preter._vignetteControlWaitKe = true;
    };
    
    
    // 操作ウェイトの解除
    function remControlWait(preter) {
        preter._vignetteControlWaitKe = false;
    };
    ...
    function childVignette(sprite) {
        ...
        const match = vignette.layer.match(/レイヤー([AB])(\d+)/);
        if (!match) { return; }
        const layers = match[1].includes("A") ? scene._vignetteLayersKeA : scene._vignetteLayersKeB;
    ...
    function initVignetteSprites(i) {
        const gt = $gameTemp;
        if (!gt._vignetteSpritesMapKe) { gt._vignetteSpritesMapKe = []; };
    };
    
    
    //- ビネットスプライテスの取得
    function vignetteSprites(sceneType, scene) {
        const gt = $gameTemp;
        if (!gt._vignetteSpritesMapKe) { gt._vignetteSpritesMapKe = []; }
        return gt._vignetteSpritesMapKe;
    };
    
    
    //- ビネットスプライテスのセット
    function setVignetteSprites(val, sceneType) {
        const gt = $gameTemp;
        gt._vignetteSpritesMapKe = val;
    };
    
    
    //- ビネットスプライトの仮チルド-マップ
    function tempChildVignetteSpriteMap() {
        const sprites = $gameTemp._vignetteSpritesMapKe;
        if (sprites) {
    ...
    function tempChildVignetteSprite(sprite, force) {
        if (!sprite) { return; }
        if (!force && sprite._vignetteKe.takeOver) { return; }
        const gt = $gameTemp;
    ...
    function convertVariable(str) {
        if (!str) { return str; }
        const matches = str.match(/\\v\[(\d+)\]/g);
        if (!matches) { return str; }
        matches.forEach(parts => {
            const match = parts.match(/\\v\[(\d+)\]/);
    ...
    function startVignette(args) {
        ...
        // フェードイン
        p.fadeTime = Number(args["フェードイン時間"]);
        // フェードのセット
        ...
        // 同じ名前のビネットを消去
        delVignette(gm, null, p.name, p.fadeTime)
        // 変数セット
        p.wentX = 0;
        p.wentY = 0;
        ...
        // 下に挿入
            ...
            const sprites = vignetteSprites(sceneType);
    ...
    function convertVignetteParamForm(p, args) {
        ...
        if (cfg && Object.keys(cfg).length) {        
            params = ["squareRoundLine", "squareRoundEdge", "ellipseAngle", "diyaRound", "gizaOutX", "gizaOutY", "beltRound"];
            words = ["スクエア-線丸み", "スクエア-角丸み", "楕円-回転角", "ダイヤ-線丸み", "ギザギザ-トゲ長さX", "ギザギザ-トゲ長さY", "帯-線丸み"];
            params.forEach((param, i) => {
                const word = words[i];
                p[param] = cfg[word] ? (Number(cfg[word]) || 0) : p[param];
            });
        }
    };
    ...
    function setVignetteAnime(vignette, args) {
        ...
        params.forEach((param, i) => {
            ...
            anime.timeMax = news[word+"アニメ-時間"] ? Number(news[word+"アニメ-時間"]) : anime.timeMax;
            anime.dura = 0;
    ...
    function setVignetteChase(vignette, args) {
        const news = args["追尾設定"] ? JSON.parse(args["追尾設定"]) : null;
        if (!news) { return; }
        // 追尾を解除
        if (eval(news["追尾解除"])) {
            vignette.chase = null;
        ...
        const charas = [...getCharasByName(news["対象キャラ-名前"]), ...getCharasById(news["対象キャラ-ID"])];
    ...
    function createVignetteSprite(spriteset, sceneType, vignette, i) {
        ...
        sprites = vignetteSprites(sceneType);
        // 変数セット
        sprites[i] = sprite;
        // アンカー
        sprite.anchor.x = 0.5;
    ...
    function createVignetteBitmap(vignette) {
        ...
        params.forEach(param => {
            forms[param] = vignette[param] || 0;
        });
        // 各フォームを描画
        const beltR = forms.beltRound;
        if (form == "スクエア") {
            designInSquare(bitmap,  0,  -blurH,  aWidth,  aHeight,  squareW,  squareH, forms.squareRoundLine,  forms.squareRoundEdge);
        } else if (form == "真円") {
            designInCircle(bitmap,  0,  -blurH,  aWidth,  aHeight,  Math.max(lengthX, 0) / 2,  Math.max(lengthY, 0) / 2, 0, Math.PI * 2, "arc");
        } else if (form == "楕円") {
            designInCircle(bitmap,  0,  -blurH, aWidth,  aHeight,  Math.max(lengthX, 0) / 2,  Math.max(lengthY, 0) / 2, 0,  Math.PI * 2,  "ellipse",  forms.ellipseAngle);
        } else if (form == "横アーモンド") {
            ...
            designInDiya(bitmap,  0,  -blurH,  aWidth,  aHeight,   squareW, squareH, forms.diyaRound);
        } else if (form.includes("ギザギザ")) {
            const smooth = form.includes("滑らか");
            designInGiza(bitmap,  0,  -blurH,  aWidth,  aHeight,  squareW,  squareH, formCfg, smooth, forms.gizaOutX, forms.gizaOutY);
        } else if (form == "上下帯") {
    ...
    function rechildVignette(scene) {
        const sprites = vignetteSprites(null, scene);
        if (sprites) {
            sprites.forEach(sprite => {
                if (!sprite) { return; }
                const vnt = sprite._vignetteKe;
                if (vnt.scene != scene.constructor.name && !vnt.takeOver) { return; }
    ...
    function updateVignette(spriteset, sceneType) {
        ...
        if (del) {
            gm._vignettesKe = vignettes.filter(vnt => vnt);
            setVignetteSprites(vignetteSprites(sceneType).filter(s => s), sceneType);
        }
    };
    ...
    function updateVignetteSprite(spriteset, sceneType, vignette, i) {
        ...
        // スケール
        sprite.scale.x = vignette.scaleX * vignette.animeScale;
        sprite.scale.y = vignette.scaleY * vignette.animeScale;
        // 回転角
        sprite.rotation = vignette.anglePi + vignette.animeAnglePi;
        // 位置
    ...
    function chaseVignetteSprite(sceneType, sprite, vignette, i) {
        ...
        const tageY = target.screenY();
        sprite.x = tageX + vignette.offsetX;
        sprite.y = tageY + vignette.offsetY;
    };
    ...
    function startIrisEffect(args) {
        ...
        if (!vignettes || !vignettes.length) { remIrisWait(args.preter); return; }
        // アイリスエフェクトの1個制限
    ...
    function limitIrisEffectOne(args, handler, vignettes = [null]) {
        ...
        // 他がアイリス中なら予約してリターン
        if ((gm._inIrisEffectKe || !SceneManager._scene._spriteset) && !(vignette && vignette.irisEffect)) {
            // アイリス予約のセット
    ...
    function updateIrisAppo(gm) {
        ...
        if (gm._inIrisEffectKe) { return; }
        const appo = gm._irisApposKe.shift();
    ...
    function startVignetteControl(args) {
        ...
        if (!targets || !targets.length) { remControlWait(args.preter); return; }
        // アイリスパラムの変換-基本
        const ctrl = {};
    ...
    function convertIrisParamBasic(p, args, toRoute) {
        p.timeMax = toRoute ? Number(args["…アニメ枚数"]) || 1 : Number(args["動作時間"]) || 1;
        p.dura = 0;
    ...
    function updateVignetteControl(spriteset, sceneType, vignette, i) {
        if (!vignette.vntControl) { return; }
        const ctrl = vignette.vntControl
        // スプライテスを取得
        const sprites = vignetteSprites(sceneType);
        // カウント
        ctrl.dura++;
        ...
        if (ctrl.dura >= ctrl.timeMax) {
            // ビネット操作の終了
            endVignetteControl(vignette, ctrl, i);
        }
    };
    ...
    //- ビネット操作の終了
    function endVignetteControl(vignette, ctrl, i) {
        vignette.vntControl = null;
        // 操作ウェイトの解除
    ...
    //- キャッシュ予約のセット
    /*function setCashAppo(args, handler, callName, vignettes) {
        const gm = getGm();
        if (!gm._irisCashApposKe) { gm._irisCashApposKe = []; }
        const appo = { args:args, handler:handler, callName:callName, vignettes:vignettes };
        gm._irisCashApposKe.push(appo);
    };
    
    
    //- キャッシュ予約の更新
    function updateCashAppo(gm) {
        if (!gm._irisCashApposKe || !gm._irisCashApposKe.length) { return; }
        if (!SceneManager._scene._spriteset) { return; }
        const appos = gm._irisCashApposKe;
        // 各予約を実行できるか判定
        let del = false;
        appos.forEach((appo, i) => {
            appo.handler(appo.args, appo.targets);
        });
        gm._irisCashApposKe = null;
    };*/
    ...
    //- キャッシュの破棄
    function destroyCash(callName, lastIndex, omitOnly) {
        if (!ImageManager._irisCashsKe) { return; }
        const cashs = ImageManager._irisCashsKe;
        if (!cashs[callName]) { return; }
        const cash = cashs[callName];
        // 必要なら、最後のキャッシュを除外
    ...
    function eraseVignette(args) {
        const gm = getGm();
        if (!gm._vignettesKe) { return }
        const fadeTime = Number(args["フェードアウト時間"]);
        // すべて削除
    ...
    function getCharasByName(names, self) {
        ...
        for (const name of nameList) {
            ...
            // フォロワーを取得
            if (name.match(/^(フォロワー|フォロアー|隊列|隊列キャラ|follower)$/)) {
                charas = [...charas, ...$gamePlayer._followers._data];
            }
            // パーティを取得
            ...
            // 乗り物を取得
            if (name.match(/^(乗り物|乗物|乗機|vehicle)/)) {
                let id = name.match(/\d+/);
                id = id ? Number(id[0]) - 1 : 0;
                charas.push($gameMap._vehicles[id]);
            }
            // 全て取得
    ...
    function getCharasById(ids, self) {
        ...
        for (const id of idList) {
            ...
            // フォロワーを取得
            if (id == -3) { charas = [...charas, ...$gamePlayer._followers._data]; }
            // 乗り物を取得
            if (id <= -11) {
                charas.push($gameMap._vehicles[Math.abs(id) - 11]);
            }
            // 全て取得
            if (id == -2) {
                charas = [...$gameMap.events(), $gamePlayer, ...$gamePlayer._followers._data, ...$gameMap._vehicles];
    ...
    //- 中抜きサークルのデザイン
    function designInCircle(bitmap, x, y, w, h, radiusX, radiusY, start, end, type, angle = 0) {
        const context = bitmap.context;
        context.beginPath();
        ...
        context.lineTo(x2,  y3);
        if (type == "arc") {
            context.arc(x3,  y3,  radiusX,  Math.PI,  0);
        } else {
            context.ellipse(x3,  y3,  radiusX,  radiusY,  radian,  Math.PI,  0);
        }
        ...
        context.lineTo(x4,  y3);
        if (type == "arc") {
            context.arc(x3,  y3,  radiusX,  0,  Math.PI);
        } else {
            context.ellipse(x3,  y3,  radiusX,  radiusY,  radian,  0,  Math.PI);
        }
        context.lineTo(x,  y3);
        context.lineTo(x,  y5);
        context.lineTo(x5,  y5);
